...
HeaterProtection::HeaterProtection(size_t index) : next(nullptr)
{
	...
	heater = supervisedHeater = (index >= MaxHeaters) ? -1 : (int8_t)index;
}
...
bool HeaterProtection::Check()
{
	if (supervisedHeater >= 0)
	{
		...
		if (err != TemperatureError::success)
		{
			...
			{
				Platform::MessageF(ErrorMessage, "Temperature reading error on heater %d\n", supervisedHeater);
				return false;
			}
		}
		else
