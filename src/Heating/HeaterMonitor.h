...
public:
	...
	bool Check(uint32_t maxBadTemperatureCount) noexcept;				// Check if any action needs to be taken
	...
	void SetAction(HeaterMonitorAction newAction) noexcept;				// Set the action to trigger when a temperature event occurs

	HeaterMonitorTrigger GetTrigger() const noexcept { return trigger; }	// Get the condition for a temperature event
	void SetTrigger(HeaterMonitorTrigger newTrigger) noexcept;			// Set the condition for a temperature event
...
inline void HeaterMonitor::SetSensorNumber(int sn) noexcept
{
	sensorNumber = sn;
	badTemperatureCount = 0;
}

inline void HeaterMonitor::SetTemperatureLimit(float newLimit) noexcept
{
	limit = newLimit;
}

inline void HeaterMonitor::SetAction(HeaterMonitorAction newAction) noexcept
{
	action = newAction;
}

inline void HeaterMonitor::SetTrigger(HeaterMonitorTrigger newTrigger) noexcept
{
	trigger = newTrigger;
}
