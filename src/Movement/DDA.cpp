...
void DDA::DebugPrintAll() const
{
	...
	for (size_t axis = 0; axis < numAxes; ++axis)
	{
		...
		{
			pddm[axis]->DebugPrint(GCodes::GetAxisLetters()[axis], isDeltaMovement);
		}
	}
...
bool DDA::Init(GCodes::RawMove &nextMove, bool doMotorMapping)
{
	...
	const float * const normalAccelerations = Platform::Accelerations();
	...
	for (size_t drive = 0; drive < DRIVES; drive++)
	{
		...
		{
			directionVector[drive] = (float)delta/Platform::DriveStepsPerUnit(drive);
		...
		if (delta != 0)
		{
			...
			if (drive >= numTotalAxes)
			{
				...
				{
					const float compensationTime = Platform::GetPressureAdvance(drive - numTotalAxes);
					if (compensationTime > 0.0)
					{
						...
						accelerations[drive] = min<float>(accelerations[drive], Platform::GetInstantDv(drive)/compensationTime);
					}
				}
			}
			else
...
//	filePos = nextMove.filePos;
	...
	requestedSpeed = min<float>(max<float>(reqSpeed, MinimumMovementSpeed), VectorBoxIntersection(normalisedDirectionVector, Platform::MaxFeedrates(), DRIVES));
...
{
	// 1. Compute the new endpoints and the movement vector
	const float ZAcceleration = Platform::Accelerations()[Z_AXIS];
	const float ZSpeed = Platform::MaxFeedrate(Z_AXIS);
	...
	for (size_t drive = 0; drive < DRIVES; drive++)
	{
		...
		const int32_t delta = lrintf(directionVector[drive] * Platform::DriveStepsPerUnit(Z_AXIS));
...
{
//	if (Platform::Debug(moduleDda)) debugPrintf("Adjusting, %f\n", laDDA->targetNextSpeed);
...
LA_DEBUG;
			...
			if (laDepth == 0)
			{
//				if (Platform::Debug(moduleDda)) debugPrintf("Complete, %f\n", laDDA->targetNextSpeed);
				return;
			}
...
float DDA::AdvanceBabyStepping(float amount)
{
	...
	while(cdda != this)
	{
		...
		if (cdda->isDeltaMovement)
		{
			...
			{
				cdda->endPoint[tower] += (int32_t)(babySteppingDone * Platform::DriveStepsPerUnit(tower));
				if (babySteppingToDo != 0.0)
				{
					int32_t steps = (int32_t)(babySteppingToDo * Platform::DriveStepsPerUnit(tower));
		...
		{
			cdda->endPoint[Z_AXIS] += (int32_t)(babySteppingDone * Platform::DriveStepsPerUnit(Z_AXIS));
			if (babySteppingToDo != 0.0)
			{
				int32_t steps = (int32_t)(babySteppingToDo * Platform::DriveStepsPerUnit(Z_AXIS));
...
void DDA::RecalculateMove()
{
		...
		else
		{
			...
			if (startSpeed < endSpeed)
			{
				...
				if (newAcceleration > 1.02 * acceleration)
				{
					...
					moveInstance->RecordLookaheadError();
	...
	if (canPauseAfter && endSpeed != 0.0)
	{
		for (size_t drive = 0; drive < DRIVES; ++drive)
		{
			if (pddm[drive] != nullptr && pddm[drive]->state == DMState::moving && endSpeed * fabsf(directionVector[drive]) > Platform::GetInstantDv(drive))
			{
				canPauseAfter = false;
				break;
			}
...
void DDA::MatchSpeeds()
{
	...
	for (size_t drive = 0; drive < DRIVES; ++drive)
	{
		   ...
		   )
		{
			...
			const float allowedJerk = Platform::GetInstantDv(drive);
			if (jerk > allowedJerk)
			{
...
bool DDA::FetchEndPosition(volatile int32_t ep[DRIVES], volatile float endCoords[DRIVES])
{
	...
	for (size_t eDrive = GCodes::GetTotalAxes(); eDrive < DRIVES; ++eDrive)
	{
...
{
	moveInstance->EndPointToMachine(move, endPoint, numDrives);
...
pre(disableDeltaMapping || drive < MaxAxes)
{
	...
	else
	{
		...
		{
			moveInstance->MotorStepsToCartesian(endPoint, visibleAxes, GCodes::GetTotalAxes(), endCoordinates);
			endCoordinatesValid = true;
		}
...
inline void DDA::AdjustAcceleration()
{
	...
	const float idealPeriod = moveInstance->GetDRCperiod();
	...
	{
		const float drcMinimumAcceleration = moveInstance->GetDRCminimumAcceleration();
...
#if SUPPORT_LASER
	if (topSpeed < requestedSpeed && GCodes::GetMachineType() == MachineType::laser)
	{
	...
	if (simMode == 0)
	{
		...
		for (size_t drive = 0; drive < DRIVES; ++drive)
		{
			...
			if (pdm != nullptr && pdm->state == DMState::moving)
			{
				...
				{
					Platform::EnableDrive(Z_AXIS);			// ensure all Z motors are enabled
		...
		if (IsBitSet(endStopsToCheck, drive))
		{
			...
			case EndStopHit::highHit:
				...
				else
				{
					...
					if (true)
#else
					const Kinematics& kin = moveInstance->GetKinematics();
					...
					{
						kin.OnHomingSwitchTriggered(drive, esh == EndStopHit::highHit, Platform::GetDriveStepsPerUnit(), *this);
...
pre(state == frozen)
{
	...
	if (firstDM != nullptr)
	{
		...
		{
			const unsigned int prohibitedMovements = Platform::GetProhibitedExtruderMovements(extrusions, retractions);
...
float DDA::GetProportionDone(bool moveWasAborted) const
{
	...
	if (moveWasAborted)
	{
		...
		if (proportionDoneAtEnd > proportionDone)
		{
			...
			for (size_t drive = GCodes::GetTotalAxes(); drive < DRIVES; ++drive)
			{
