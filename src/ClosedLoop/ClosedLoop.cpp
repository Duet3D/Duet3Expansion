...
void ClosedLoop::Diagnostics(const StringRef& reply) noexcept
{
	...
	if (encoder != nullptr)
	{
		...
		{
			reply.catf(", position %" PRIi32, reading);
		}
...
bool ClosedLoop::SetClosedLoopEnabled(size_t driver, bool enabled, const StringRef &reply) noexcept
{
	...
	if (enabled && !closedLoopEnabled)
	{
		...
		encoder->AdjustOffset(lrintf(((int32_t)initialStepPhase - (int32_t)measuredStepPhase) * encoder->GetCountsPerStep() / 1024.0));	// set the new zero position
